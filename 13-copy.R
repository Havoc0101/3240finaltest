...
ui <- fluidPage(
  ...
  div(
    ...
    h1("Global Air Quality Research Platform"),
    
    div(
      class = "sub-title",
      "PM2.5 Distribution & Pollutant Analysis"
    ),
    
    div(
      class = "island-buttons",
      
      # Button 1: switch to the data-cleaning summary page
      actionButton("go_cleaning", "Cleaning", class = "island-btn"),
      
      # Button 2: switch to the pollutant correlation heatmap page
      actionButton("go_correlation", "Correlation", class = "island-btn"),
      
      # Button 3: switch to the PM2.5 country ranking page
      actionButton("go_ranking", "Ranking", class = "island-btn"),
      
      # Button 4: switch to the country PM2.5 profile page
      actionButton("go_profile", "Profile", class = "island-btn"),
      
      # Button 5: switch to the global PM2.5 map page
      actionButton("go_map", "Map", class = "island-btn"),
      
      # Button 6: switch to the GDP + geography radar page
  ...
  tags$head(
    ...
    body {
      ...
      background-image:
        radial-gradient(circle, #e0e8ff 7%, transparent 8%),
        radial-gradient(circle, #ffe8f0 7%, transparent 8%);
    ...
    /* =========================
       Hover animation
       Make buttons feel interactive (Dynamic Island effect)
       ========================= */
    .island-btn:hover {
    ...
    tabPanel(
      ...
      fluidRow(
        ...
        column(
          ...
          # Modern card for radar chart and selected-country map
          div(
            class = "modern-card",
...
server <- function(input, output, session) {
  ...
  # When "Cleaning" button is clicked → switch to cleaning page
  observeEvent(input$go_cleaning, {
    updateTabsetPanel(session, "main_tabs", selected = "Cleaning")
  })
  
  # When "Correlation" button is clicked → switch to correlation page
  observeEvent(input$go_correlation, {
    updateTabsetPanel(session, "main_tabs", selected = "Correlation")
  })
  
  # When "Ranking" button is clicked → switch to ranking page
  observeEvent(input$go_ranking, {
    updateTabsetPanel(session, "main_tabs", selected = "Ranking")
  })
  
  # When "Profile" button is clicked → switch to profile page
  observeEvent(input$go_profile, {
    updateTabsetPanel(session, "main_tabs", selected = "Profile")
  })
  
  # When "Map" button is clicked → switch to map page
  observeEvent(input$go_map, {
    updateTabsetPanel(session, "main_tabs", selected = "Map")
  })
  
  # When "Radar" button is clicked → switch to radar page
  observeEvent(input$go_radar, {
    updateTabsetPanel(session, "main_tabs", selected = "Radar")
  })
  ...
  # =========================
  observe({
    shinyjs::removeClass(selector = ".island-btn", class = "active")
    
    if (input$go_cleaning > 0) shinyjs::addClass("go_cleaning", "active")
    if (input$go_correlation > 0) shinyjs::addClass("go_correlation", "active")
    if (input$go_ranking > 0) shinyjs::addClass("go_ranking", "active")
    if (input$go_profile > 0) shinyjs::addClass("go_profile", "active")
    if (input$go_map > 0) shinyjs::addClass("go_map", "active")
    if (input$go_radar > 0) shinyjs::addClass("go_radar", "active")
  })
